<template>
  <div>
    ...
    >
      ...
      <div>{{ pedido.nome }}</div>
      <div>{{ pedido.burguer ? pedido.burguer.nome : '-' }}</div>
...
export default {
  ...
  methods: {
    ...
    async deletarPedido(id) {
      ...
      if(response.ok) {
        this.msg = "Info: Pedido removido com sucesso!";
        this.msgType = "info";
        setTimeout(() => this.msg = "", 3000);
        this.consultarPedidos(); // Atualização real-time
      }
    },
    async atualizarStatusPedido(event, idPedido) {
      const idStatusAtualizado = event.target.value;
      const atualizaoJson = JSON.stringify({ statusId: idStatusAtualizado });
      ...
      if(response.ok) {
        const statusObj = this.listaStatusPedido.find(s => s.id == idStatusAtualizado);
        this.msg = `Sucesso: Status do pedido Nº ${idPedido} atualizado para ${statusObj.descricao}!`;
